...
func RegisterRoutes(r *gin.Engine, repos *repository.Repositories, deps RouteDependencies) {
	...
	discoverer, _ := scannerSvc.(service.CareerBoardDiscoverer)
	companies := NewCompanyHandler(service.NewCompanyService(repos.Company, scannerSvc), discoverer)
	...
	protected.Use(Authenticated(authSvc))
	{
		...
		protected.POST("/companies/discover", companies.Discover)
		protected.POST("/companies/register", companies.RegisterBoards)
		protected.PATCH("/companies/:id", companies.UpdateActive)
		protected.PATCH("/companies/:id/details", companies.UpdateDetails)
		protected.DELETE("/companies/:id", companies.Delete)
		protected.POST("/companies/:id/boards", companies.CreateBoard)
		protected.PATCH("/companies/:id/boards/:boardID", companies.UpdateBoardActive)
		protected.PATCH("/companies/:id/boards/:boardID/details", companies.UpdateBoardDetails)
		protected.DELETE("/companies/:id/boards/:boardID", companies.DeleteBoard)
