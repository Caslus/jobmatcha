...
// later cooldown advertised by the response.
func (c *ProviderHTTPClient) Do(req *http.Request) (*http.Response, error) {
	...
	if resp != nil {
		c.coordinator.observe(c.provider, resp.Header)
	}
	return resp, err
}
...
func (c *RateLimitCoordinator) observe(provider string, header http.Header) {
